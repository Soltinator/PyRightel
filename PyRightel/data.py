...
class session:
    def __init__(self):
        ...
        if static.httpAdapter is None:
            static.httpAdapter = requests.adapters.HTTPAdapter(
                max_retries=static.retryStrategy
            )
        self.session.mount(static.rootUrl, static.httpAdapter)
        self.authExpirey = None
        self.headers = {
            "version": static.apiVersion,
...
class package:
    def __init__(self):
        self.packageName: (
            str  # friendly name of the package when bought. not an id or unique name.
        )
...
class static:
    ...
    retryStrategy = Retry(
        total=maxRetryOnError, redirect=0, backoff_factor=1, backoff_max=5
    )
